...
struct Column {
  static inline void compute(const Y_Mat_Type &Y_horizon,
    ...
    const auto ymin = Y_min_matrix.template get<I, J_idx>();
    const auto ymax = Y_max_matrix.template get<I, J_idx>();

    if (y < ymin) {
      Y_limit_penalty.template set<I, J_idx>(y - ymin);
    } else if (y > ymax) {
      Y_limit_penalty.template set<I, J_idx>(y - ymax);
    } else {
      /* Do Nothing */
    }
...
struct Column<Y_Mat_Type, Y_Min_Matrix_Type, Y_Max_Matrix_Type, Out_Type, M, N,
  ...
  static inline void compute(const Y_Mat_Type &Y_horizon,
    ...
    const auto ymin = Y_min_matrix.template get<I, 0>();
    const auto ymax = Y_max_matrix.template get<I, 0>();

    if (y < ymin) {
      Y_limit_penalty.template set<I, 0>(y - ymin);
    } else if (y > ymax) {
      Y_limit_penalty.template set<I, 0>(y - ymax);
    } else {
      /* Do Nothing */
    }
  }
